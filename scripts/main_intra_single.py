...
class Trainer:
    def __init__(self, args, device):
        ...
        # data loader
        self.source_loader, self.val_loader, self.target_loader = get_intra_single_loader(args)
    ...
    def _do_student(self, epoch=None):
        ...
        self._print('Eps:{}/{} | s_cls_loss={} | ad_loss={} | acc={} | BS:{} | Lr:{}'.format(
        ...
        if self.s_results['test'][epoch] > self.s_best_test[self.args.target_idx]:
            ...
            outfile = os.path.join(self.log_dir, '{}.tar'.format(self.args.target))
...
def main():
    ...
    '''--------------------------------------------'''
    args.name = args.suffix + '/intra_adr_single'
    random.seed(0)
    np.random.seed(0)
